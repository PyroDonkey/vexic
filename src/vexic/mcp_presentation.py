...
RECALL_USER_MEMORY_DESCRIPTION = (
    ...
    "even if they don't mention memory, and before telling them you don't know "
    "or remember something, or that a value, name, or fact they ask you to "
    "recall doesn't exist or is made-up — search first. Returns verified facts, "
    "or clearly marked tentative "
...
def server_instructions(enable_expand_history: bool = False) -> str:
    ...
    return (
        ...
        "the user; or you are about to tell the user that something they may "
        "have shared, asked you to remember, or discussed before doesn't exist, "
        "isn't in memory, is made-up, or that you can't recall it — search "
        "first. A search that returns nothing lets you give that honest "
        "negative; search before denying, don't refuse to deny. Use "
        "recall_user_memory for durable facts and "
        "preferences, recall_conversation_history for what was said in current "
        "or earlier conversations.\n"
        "\n"
        "SESSION-START PRIMING: a snapshot of this user's memory may already "
        "be injected into context at session start (marked 'Vexic memory "
        "priming:'). The snapshot is a sample, not the whole memory. Facts "
        "already visible there need no re-search, but this never weakens "
        "the search-first rule above: before telling the user something "
        "doesn't exist, isn't remembered, or can't be recalled, search even "
        "if the snapshot doesn't mention it. Snapshot items ending in '…' or "
        "marked '[truncated]' are incomplete — use the recall tools to "
        "retrieve the full text before relying on them.\n"
        "\n"
